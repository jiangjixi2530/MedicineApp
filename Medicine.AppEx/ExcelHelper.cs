...
namespace Medicine.AppEx
{
    public class ExcelHelper
    {
        ...
        {
            return ExcelToList<T>(workBook.GetSheetAt(sheetIndex));
        }
        /// <summary>
        /// Sheet中的数据转换为List集合
        /// </summary>
        /// <param name="sheet"></param>
        /// <returns></returns>
        public IList<T> ExcelToList<T>(ISheet sheet) where T : class, new()
        {
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    if (info != null)
                    {
                        ...
                        {
                            info.SetValue(s, 0, null);
                            //throw new Exception("字段[" + info.Name + item[i] + "]转换出错," + ex.Message);
                        }
                    }
                }
                list.Add(s);
        ...
        {
            var index = workBook.GetSheetIndex(sheet);
            if (index < 0)
            {
                return;
            }
